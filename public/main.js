...
var equipInterval = null;
var lastEquipPayload = null;
var agentSelectWatcher = null;
...
ipcMain.on('equip', async (event, skinUid, autoRefresh) => {
    if (!axiosClient) return mainWindow.webContents.send('unauthorized');
    if (!playerUrl) {
    ...
    if (!loadout) { mainWindow.webContents.send('equipResult', { success: false }); return console.log('Could not fetch loadout'); }

    const weaponsRes = await axios.get('https://valorant-api.com/v1/weapons').catch(e => null);
    if (!weaponsRes) { mainWindow.webContents.send('equipResult', { success: false }); return console.log('Could not fetch weapons from valorant-api'); }

    const gun = weaponsRes.data.data.find(weapon =>
        weapon.skins.some(skin => skin.uuid.toLowerCase() === skinUid.toLowerCase())
    );
    if (!gun) { mainWindow.webContents.send('equipResult', { success: false }); return console.log('Gun not found for skin:', skinUid); }

    const skinData = gun.skins.find(skin => skin.uuid.toLowerCase() === skinUid.toLowerCase());
    const guns = loadout.Guns || loadout.guns;
    if (!guns) { mainWindow.webContents.send('equipResult', { success: false }); return console.log('No Guns array in loadout response'); }

    const gunInLoadout = guns.find(g => (g.ID || g.id)?.toLowerCase() === gun.uuid.toLowerCase());
    if (!gunInLoadout) { mainWindow.webContents.send('equipResult', { success: false }); return console.log('Gun not found in loadout:', gun.uuid); }

    const skinLevelId = skinData.levels?.[skinData.levels.length - 1]?.uuid || gunInLoadout.SkinLevelID;
    const skinChromaId = skinData.chromas?.[0]?.uuid || gunInLoadout.ChromaID;

    gunInLoadout.SkinID = skinData.uuid;
    gunInLoadout.SkinLevelID = skinLevelId;
    gunInLoadout.ChromaID = skinChromaId;

    const putLoadout = async () => {
        for (const ver of ['v2', 'v3']) {
            try {
                await axios.put(`${playerUrl}/personalization/${ver}/players/${playerUUid}/playerloadout`, loadout, { headers: pdHeaders });
                console.log(`Loadout PUT via REMOTE ${ver} OK`);
                return ver;
            } catch (err) {
                console.log(`Remote ${ver} failed:`, err.response?.status, err.message);
            }
        }
        return null;
    };

    const putVersion = await putLoadout();

    if (putVersion) {
        lastEquipPayload = { loadout, skinName: skinData.displayName };
        mainWindow.webContents.send('equipResult', { success: true, name: skinData.displayName });
    } else {
        mainWindow.webContents.send('equipResult', { success: false });
    }
})

ipcMain.on('toggleAutoRefresh', (event, enable) => {
    if (equipInterval) {
        clearInterval(equipInterval);
        equipInterval = null;
        console.log('Auto-refresh STOPPED');
    }
    if (enable && lastEquipPayload) {
        const pdHeaders = {
            'Authorization': 'Bearer ' + accessToken,
            'X-Riot-Entitlements-JWT': entitlementsToken,
            'X-Riot-ClientVersion': riotClientVersion,
        ...
        equipInterval = setInterval(async () => {
            try {
                await axios.put(`${playerUrl}/personalization/v3/players/${playerUUid}/playerloadout`, lastEquipPayload.loadout, { headers: pdHeaders });
                console.log('Auto-refresh: PUT OK');
            } catch (e) {
                console.log('Auto-refresh failed:', e.message);
            }
        }, 2000);
        console.log('Auto-refresh STARTED (every 2s)');
    }
})

ipcMain.on('toggleAgentSelect', (event, enable) => {
    if (agentSelectWatcher) {
        clearInterval(agentSelectWatcher);
        agentSelectWatcher = null;
        console.log('Agent select watcher STOPPED');
    }
    if (enable) {
        agentSelectWatcher = setInterval(async () => {
            if (!axiosClient || !lastEquipPayload) return;
            try {
                const res = await axiosClient.get('/chat/v4/presences');
                const me = res.data.presences?.find(p => p.puuid === playerUUid);
                if (!me) return;
                const privateData = JSON.parse(Buffer.from(me.private, 'base64').toString('utf8'));
                const state = privateData?.sessionLoopState;
                if (state === 'PREGAME' || state === 'INGAME') {
                    console.log(`Agent select: state=${state}, pushing loadout...`);
                    try {
                        await axios.put(`${playerUrl}/personalization/v3/players/${playerUUid}/playerloadout`, lastEquipPayload.loadout, {
                            headers: {
                                'Authorization': 'Bearer ' + accessToken,
                                'X-Riot-Entitlements-JWT': entitlementsToken,
                                'X-Riot-ClientVersion': riotClientVersion,
                        ...
                        console.log('Agent select: PUT OK');
                    } catch (e) {
                        console.log('Agent select: PUT failed', e.message);
                    }
                }
            } catch (e) {}
        }, 1500);
        console.log('Agent select watcher STARTED (checking every 1.5s)');
    }
})
