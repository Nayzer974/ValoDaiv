...
const Skins = () => {
  ...
  const equip = (uuid, name) => {
    ...
    ipcRenderer.once('equipResult', (event, result) => {
      if (result.success) {
        Swal.fire({ icon: 'success', text: `Equipped: ${result.name || name}`, toast: true, position: 'top-end', background: '#141418', color: '#f0f0f8', showConfirmButton: false, timer: 2500, iconColor: '#ff4655' })
      } else {
        Swal.fire({ icon: 'error', text: 'Failed to equip skin', toast: true, position: 'top-end', background: '#141418', color: '#f0f0f8', showConfirmButton: false, timer: 2500, iconColor: '#ff4655' })
      }
    })
  }
  ...
  const toggleAutoRefresh = () => {
    ...
    setAutoRefresh(next)
    ipcRenderer.send('toggleAutoRefresh', next)
    Swal.fire({
      icon: next ? 'success' : 'info',
      text: next ? 'Auto-refresh ON (every 2s)' : 'Auto-refresh OFF',
      toast: true, position: 'top-end', background: '#141418', color: '#f0f0f8',
      showConfirmButton: false, timer: 2000, iconColor: next ? '#46ff90' : '#ff4655'
    })
  }

  const toggleAgentWatch = () => {
    const next = !agentWatch
    setAgentWatch(next)
    ipcRenderer.send('toggleAgentSelect', next)
    Swal.fire({
      icon: next ? 'success' : 'info',
      text: next ? 'Agent Select watcher ON' : 'Agent Select watcher OFF',
      toast: true, position: 'top-end', background: '#141418', color: '#f0f0f8',
      showConfirmButton: false, timer: 2000, iconColor: next ? '#46ff90' : '#ff4655'
    })
  }
    ...
    <div style={{ width: '100%', height: '100%', display: 'flex', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '7px 16px', background: 'rgba(255,70,85,0.08)', borderBottom: '1px solid rgba(255,70,85,0.18)', flexShrink: 0 }}>
          ...
          <span style={{ color: '#c0a0a4', fontSize: 11, letterSpacing: 0.5 }}>Equip skin then enable Auto-Refresh to keep it in-game</span>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexShrink: 0 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <button onClick={toggleAgentWatch}
              className='NoDrag'
              style={{ padding: '5px 10px', borderRadius: 8, background: agentWatch ? 'rgba(255,170,46,0.12)' : '#18181e', border: `1px solid ${agentWatch ? '#ffaa2e' : '#252530'}`, color: agentWatch ? '#ffaa2e' : '#50506a', fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>
              {agentWatch ? '● AGENT' : '○ AGENT'}
            </button>
            <button onClick={toggleAutoRefresh}
              className='NoDrag'
              style={{ padding: '5px 10px', borderRadius: 8, background: autoRefresh ? 'rgba(70,255,144,0.12)' : '#18181e', border: `1px solid ${autoRefresh ? '#46ff90' : '#252530'}`, color: autoRefresh ? '#46ff90' : '#50506a', fontSize: 11, fontWeight: 600, cursor: 'pointer', transition: 'all 0.15s' }}>
